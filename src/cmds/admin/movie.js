...
module.exports = {
  ...
  execute: (message, args) => {
    ...
    const movieMsgChannel = message.client.channels.resolve(
      match.groups.channelId
    );
    if (!movieMsgChannel) return;
    movieMsgChannel.messages
      .fetch(match.groups.messageId, true)
      .then((movieMsg) => {
        ...
        message.channel.send(
          "Sikeresen létrehoztál egy jegy vásárlói üzenetet. Az üzenet törlésével tudod megállítani a jegyek vásárlását."
        );
      })
      .catch(console.error);
  },
